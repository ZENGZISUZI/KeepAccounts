...
//        String date=year+"年"+(monthOfYear+1)+"月"+dayOfMonth+"日";
        ...
        for (int i = 0; i < numberOfLines; ++i) {
           ...
           try {
                ...
                List<Acount> acountList =db.selector(Acount.class).where("date", "like", date).and("type","=",1).findAll();
                int j = 0;
            ...
            try {
                List<Acount> acountList =db.selector(Acount.class).where("date", "like", date).and("type","=",0).findAll();
                int j = 0;
                for (int t =0; t < acountList.size(); t++) {
                    values.add(new PointValue(j, acountList.get(t).getFigure()));
                    if(acountList.get(t).getFigure()>max)
