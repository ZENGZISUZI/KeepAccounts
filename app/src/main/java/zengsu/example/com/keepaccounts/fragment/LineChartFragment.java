       ...
       // chart = (LineChartView) findViewById(R.id.chart);
        ...
        for (int i = 0; i < numberOfLines; ++i) {
           ...
           /* Cursor cursor = myDatabasehelper.getReadableDatabase().rawQuery(
                ...
                List<Account> accountList =db.selector(Account.class).where("date", "=", date).and("type","=",1).findAll();
                int j = 0;
            ...
            try {
                List<Account> accountList =db.selector(Account.class).where("date", "=", date).and("type","=",0).findAll();
                int j = 0;
                for (int t = 0; t < accountList.size(); t++) {
                    values.add(new PointValue(j, accountList.get(t).getFigure()));
                    if(accountList.get(t).getFigure()>max)
